...
test("the fixture plan maps the legacy customMix flag", () => {
  ...
  // A limit below the floor is ignored rather than trusted.
  assert.deepEqual(leagueFixturePlan({ fixtureMode: "limited", fixtureLimit: 2 }), { mode: "limited", limit: null });
});

test("the configured count is a weekly default, not a cap", () => {
  const eight = effectiveFixtureCount({ fixtureMode: "limited", fixtureLimit: 8 }, 12);
  assert.equal(eight.default, 8, "the picker opens on the league default");
  assert.equal(eight.min, 3, "but the host may go down to three");
  ...
  const short = effectiveFixtureCount({ fixtureMode: "limited", fixtureLimit: 8 }, 5);
  assert.equal(short.default, 5, "the default caps to the pool");
  assert.equal(short.max, 5);
  assert.equal(short.capped, true);

  // A pool below the floor collapses min and max onto it rather than erroring.
  const tiny = effectiveFixtureCount({ fixtureMode: "limited", fixtureLimit: 8 }, 2);
  assert.deepEqual([tiny.min, tiny.max, tiny.default], [2, 2, 2]);

  // "all" plays the pool, whatever its size.
  const all = effectiveFixtureCount({ fixtureMode: "all" }, 12);
  assert.deepEqual([all.mode, all.default, all.min, all.max], ["all", 12, 12, 12]);

  // A legacy Custom Mix league has no stored preference: standard default.
  const legacy = effectiveFixtureCount({ customMix: true }, 12);
  assert.deepEqual([legacy.default, legacy.min, legacy.max], [8, 3, 12]);
});

// --- windows ----------------------------------------------------------------

test("windows are Monday-anchored in London time", () => {
  // Sat 15 Aug 2026 -> week beginning Mon 10 Aug.
  assert.equal(windowKeyFor("2026-08-15T14:00:00+01:00"), "w2026-08-10");
  assert.equal(windowKeyFor("2026-08-10T19:00:00+01:00"), "w2026-08-10", "Monday belongs to its own week");
  assert.equal(windowKeyFor("2026-08-16T15:00:00+01:00"), "w2026-08-10", "Sunday closes the week");
  assert.equal(windowKeyFor("2026-08-17T19:00:00+01:00"), "w2026-08-17", "the next Monday starts a new one");
  assert.equal(windowKeyFor("not a date"), null);
});

test("a late Sunday kickoff stays in its own week across the BST boundary", () => {
  // 22:00 UTC on Sunday is 23:00 London in BST — still Sunday, still that week.
  assert.equal(windowKeyFor("2026-08-16T22:00:00Z"), "w2026-08-10");
  // In GMT, 00:30 UTC Monday is 00:30 London Monday — the new week.
  assert.equal(windowKeyFor("2026-11-30T00:30:00Z"), "w2026-11-30");
});

test("window labels read as a human week", () => {
  assert.equal(windowLabel("w2026-08-10"), "Mon 10 – Sun 16 Aug");
...
test("periods are matchweeks for a single competition and windows for a mix", () => {
  ...
  assert.equal(periodKeyOf(fixture, true), "w2026-08-10");
});

// --- the mixed pool ---------------------------------------------------------

const mixedFixtures = [
  // Week of Mon 10 Aug: 2 PL + 3 ELC.
  ...
  // Week of Mon 17 Aug: ELC only — a week where one competition is dark.
...
test("the pool is the selected competitions inside the window", () => {
  ...
  assert.deepEqual([...mixed.keys()].sort(), ["w2026-08-10", "w2026-08-17"]);
  assert.equal(mixed.get("w2026-08-10").length, 5);
  assert.equal(mixed.get("w2026-08-17").length, 1);
...
test("a host may take more or fewer than the default in any given week", () => {
  ...
  // The floor and the pool are the only hard edges.
  assert.equal(validateSlate("custom", ids.slice(0, 2), pool, bounds).error, "select between 3 and 5 fixtures");

  // A pool smaller than the floor collapses the range onto the pool.
...
test("zero competitions is refused, as is an unknown one or a bad limit", async () => {
  ...
  await withFeeds(async () => {
    ...
    assert.match((await (await send("/league", { uid: "x", competitions: ["PL"], fixtureMode: "limited", fixtureLimit: 2 })).json()).error, /at least 3/);
  });
});
...
test("a mixed league slates, locks and scores on its own week window", async () => {
  ...
  await withFeeds(async () => {
    ...
    assert.equal(season.currentPeriod, "w2026-08-10");
    assert.equal(season.currentWindowLabel, "Mon 10 – Sun 16 Aug");
    ...
    const set = await send("/league/slate", { uid: "host", code, period: "w2026-08-10", mode: "custom", fixtureIds: ids });
    assert.equal(set.status, 200);
    assert.ok(store.has(`custom_slate:${code}:w2026-08-10`), "keyed on the window, not a matchweek");
    ...
    const nextWeek = await send("/league/slate", {
      uid: "host", code, period: "w2026-08-17", mode: "custom",
    ...
    const rewrite = await send("/league/slate", { uid: "host", code, period: "w2026-08-10", mode: "custom", fixtureIds: ids });
    ...
    const round = await (await get(e)(`/state?code=${code}&period=w2026-08-10`)).json();
    ...
    assert.equal(round.windowLabel, "Mon 10 – Sun 16 Aug");

    const cabinet = await (await get(e)(`/state?code=${code}&uid=host`)).json();
    assert.equal(cabinet.cabinet.gold, 1);
    assert.equal(cabinet.cabinet.weeks[0].period, "w2026-08-10");
